...
// Helper struct to pass data for delayed DNS-SD server start
struct DnssdDelayedStartInfo
{
    DnssdDelayedStartInfo(Dnssd::CommissioningMode mode) : mMode(mode) {}
    
    Dnssd::CommissioningMode mMode;
};

// Static callback function for the delayed start
void HandleDelayedDnssdStart(System::Layer * systemLayer, void * appState)
{
    auto * startInfo = reinterpret_cast<DnssdDelayedStartInfo *>(appState);
    if (startInfo != nullptr)
    {
        // Call the internal implementation with the preserved mode
        chip::app::DnssdServer::Instance().DoStartServer(startInfo->mMode);
        delete startInfo;
    }
}
...
void DnssdServer::StartServer(Dnssd::CommissioningMode mode)
{
    ...
    if (startInfo == nullptr)
    {
        ChipLogError(Discovery, "Failed to allocate memory for delayed DNS-SD start");
        // Fall back to immediate start
        DoStartServer(mode);
        return;
    }

    // Schedule delayed start using timer with proper delay
    CHIP_ERROR err = chip::DeviceLayer::SystemLayer().StartTimer(
        System::Clock::Milliseconds32(kDnssdStartDelayMs),
        HandleDelayedDnssdStart,
        startInfo);

    if (err != CHIP_NO_ERROR)
    {
        ChipLogError(Discovery, "Failed to schedule timer for delayed DNS-SD start: %" CHIP_ERROR_FORMAT, err.Format());
        // Fall back to immediate start
        DoStartServer(mode);
        delete startInfo;
    }
...
// Implementation of DNS-SD server start that is called after the delay
void DnssdServer::DoStartServer(Dnssd::CommissioningMode mode)
{
