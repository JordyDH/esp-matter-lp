...
struct TCPDelayedMessageInfo
{
    TCPDelayedMessageInfo(TCPBase * tcp, const PeerAddress & peerAddress, System::PacketBufferHandle && buffer, 
                         MessageTransportContext * msgContext = nullptr) :
        mTCP(tcp), mPeerAddress(peerAddress), mBuffer(std::move(buffer)), mMsgContext(msgContext)
    {}

    TCPBase * mTCP;
    PeerAddress mPeerAddress;
    System::PacketBufferHandle mBuffer;
    MessageTransportContext * mMsgContext;
};

// Static callback function for the timer
static void HandleDelayedMessage(System::Layer * layer, void * appState)
{
    TCPDelayedMessageInfo * info = static_cast<TCPDelayedMessageInfo *>(appState);
    TCPBase * tcp = info->mTCP;
    
    tcp->ProcessReceivedMessage(info->mPeerAddress, std::move(info->mBuffer), info->mMsgContext);
    delete info;
}

void TCPBase::ProcessReceivedMessage(const PeerAddress & peerAddress, System::PacketBufferHandle && buffer, 
                                     MessageTransportContext * msgContext)
{
    // This is a public method that can access the protected HandleMessageReceived method
    HandleMessageReceived(peerAddress, std::move(buffer), msgContext);
}
...
CHIP_ERROR TCPBase::ProcessSingleMessage(const PeerAddress & peerAddress, ActiveTCPConnectionState * state, size_t messageSize)
{
    ...
    constexpr uint32_t kMessageProcessingDelayMs = 5000; // Adjust value as needed
    ChipLogDetail(Inet, "TCP: Adding delay of %u ms before processing message", kMessageProcessingDelayMs);

    // Create info for the delayed processing
    TCPDelayedMessageInfo * messageInfo = new TCPDelayedMessageInfo(this, peerAddress, std::move(message), &msgContext);
    ...
    if (err != CHIP_NO_ERROR)
    {
        ...
        HandleMessageReceived(peerAddress, std::move(messageInfo->mBuffer), messageInfo->mMsgContext);
        delete messageInfo;
    }
